...
static int parse_add_device_para(unsigned long arg, char** dev_path, char** vol_name)
{
    int ret;
    char* blk_dev = NULL;
    char* vol = NULL;
    ...
    blk_dev = strndup_user(paras.dev_path, PAGE_SIZE);
    if(IS_ERR(blk_dev)){
        ret = PTR_ERR(blk_dev);
        LOG_ERR("strdup bdev failed:%d", ret);
        goto err;
    }
    vol = strndup_user(paras.vol_name, PAGE_SIZE);
    if(IS_ERR(vol)){
        ret = PTR_ERR(vol);
        LOG_ERR("strdup bdev failed:%d", ret);
        goto err;
...
static int parse_del_device_para(unsigned long arg, char** dev_path)
{
    int ret;
    char* blk_dev = NULL;
    struct agent_ioctl_del_dev paras;

    ret = copy_from_user(&paras, (struct agent_ioctl_del_dev __user*)arg, \
                          sizeof(struct agent_ioctl_del_dev));
    if(ret){
        LOG_ERR("copy agent_ioctl_add_dev para failed:%d", ret);
        goto err;
    }
...
static long ctrl_dev_ioctl(struct file* filp, unsigned int cmd, unsigned long arg)
{
    ...
    switch(cmd)
    {
        ...
        case AGENT_ADD_DEVICE:
            ...
            ret = parse_add_device_para(arg, &dev_path, &vol_name);
            LOG_INFO("dev_path:%s", dev_path);
            ret = blk_dev_protect(dev_path,vol_name);
            break;
        case AGENT_DEL_DEVICE:
            LOG_INFO("Agent ioctl del device");
            ret = parse_del_device_para(arg, &dev_path);
