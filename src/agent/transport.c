...
int tp_create(struct transport* net, const char* host)
{
    ...
    net->addr.path = kstrdup(host, GFP_KERNEL);
    if(IS_ERR(net->addr.path)){
        ret = -EFAULT; 
...
int tp_connect(struct transport* net)
{
    ...
    char* host = "127.0.0.1";
    net_addr.sin_addr.s_addr = in_aton(host);
#endif
    ...
    if(ret){
        LOG_ERR("socket setsocket:%s failed:%d", host, ret);
        return ret;
    } 
    ...
    if(ret){
        ...
        LOG_ERR("socket connect:%s failed:%d", host, ret);
        return ret;
    }
