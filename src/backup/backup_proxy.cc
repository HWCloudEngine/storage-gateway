...
StatusCode BackupProxy::create_backup(const CreateBackupReq* req, CreateBackupAck* ack)
{
    ...
    // transaction begin
    m_snap_proxy->add_backup_sync(backup_name);
    ...
    ret = m_snap_proxy->create_snapshot(&snap_req, &snap_ack);
    ...
    //transaction end
    m_snap_proxy->del_backup_sync(backup_name);
    ack->set_status(ret);
...
StatusCode BackupProxy::query_backup(const QueryBackupReq* req, QueryBackupAck* ack)
{
    ...
    ret = m_backup_inner_rpc_client->QueryBackup(vol_name, backup_name, backup_status);
    if(ret != StatusCode::sOk){
        LOG_ERROR << "query bakcup vname:" << vol_name << "failed:" << ret;
        ack->set_status(ret);
        return ret; 
    }
...
StatusCode BackupProxy::delete_backup(const DeleteBackupReq* req, DeleteBackupAck* ack)
{
    ...
    //1. delete snapshot
    string snap_name = backup_to_snap_name(backup_name);
    DeleteSnapshotReq snap_req;
    DeleteSnapshotAck snap_ack;
    snap_req.mutable_header()->set_scene(SnapScene::FOR_BACKUP);
    snap_req.set_vol_name(m_vol_name);
    snap_req.set_snap_name(snap_name);
    ret = m_snap_proxy->delete_snapshot(&snap_req, &snap_ack);
    if(ret != StatusCode::sOk){
        LOG_ERROR << "delete backup vname:" << vol_name << " bname:" << backup_name 
                  << "delete snapshot failed:" << ret;
        ack->set_status(ret);
        return ret; 
    }
    //2.delete backup
...
StatusCode BackupProxy::restore_backup(const RestoreBackupReq* req, RestoreBackupAck* ack)
{
    ...
    for(auto restore_blocks : restore_blocks_vec){
        ...
        for(int i = 0; i < block_num; i++){
            ...
            /*todo read from block store*/
            /*todo take snapshot on new block device*/
            /*todo write to new block device*/
        }
    }
