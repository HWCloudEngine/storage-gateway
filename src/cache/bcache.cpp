...
bool Bcache::get(Bkey key, shared_ptr<CEntry>& value)
{
    auto it = m_bcache.find(key);
    if(it != m_bcache.end()){
        value = it->second;
        cout << "get key: " << key.m_off << " " << key.m_len << " " << key.m_seq << "ok" << endl;
        return true;
    }
    cout << "get key: " << key.m_off << " " << key.m_len << " " << key.m_seq << "error" << endl;
    return false;
}
...
int Bcache::_cache_hit_read(off_t off, size_t len, char* buf, const vector<Bkey>& hit_keys)
{
    ...
    for(int i = 0; i < hit_keys.size(); i++){
        ...
        if(cache_type == CEntry::IN_MEM){
            ...
            shared_ptr<log_header_t> log_head = v->get_log_entry(); 
            log_header_t* plog_head = log_head.get();
...
{
    if(miss_keys.empty()){
        cout << "Warn: miss keys empty" << endl;
        return -1;
    }

    int blk_fd = open(m_blkdev.c_str(), O_RDONLY|O_CREAT|O_TRUNC, 0777);
    if(blk_fd == -1){
        cout << "Err: open:" <<m_blkdev << " error" << endl;
        return -1;
    }
    ...
    for(auto it : miss_keys){
        ...
        while(left > 0){
            ...
            if(ret == -1 || ret == 0){
                cout << "Err: pread err: " << errno << " ret:"<< ret<< endl;
                break;
            }
            left -= ret;
            read += ret;
...
int Bcache::read(off_t off, size_t len, char* buf)
{
    ...
    cout << "read region hit order hit keys" << endl;
