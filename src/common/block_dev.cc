...
int BlockDevice::dev_open() {
    fd_ = ::open(path_.c_str(), O_WRONLY|O_SYNC);
    if (fd_ < 0){
...
ssize_t BlockDevice::aio_write(const char* buf, size_t len,
                               off_t off, void* ioctx) {
    return 0;
}

ssize_t BlockDevice::aio_read(char* buf, size_t lne,
                              off_t off, void* ioctx) {
    return 0;
}
