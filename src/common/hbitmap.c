...
{
    HBitmap *hb = (HBitmap*)malloc(sizeof(HBitmap));
    unsigned i;
    ...
    for (i = HBITMAP_LEVELS; i-- > 0; ) {
        size = MAX((size + BITS_PER_LONG - 1) >> BITS_PER_LEVEL, 1);
        hb->sizes[i] = size;
        hb->levels[i] = (unsigned long*)malloc(size * sizeof(unsigned long));
    }
...
void hbitmap_truncate(HBitmap *hb, uint64_t size)
{
    ...
    for (i = HBITMAP_LEVELS; i-- > 0; ) {
        ...
        old = hb->sizes[i];
        hb->sizes[i] = size;
        hb->levels[i] = realloc(hb->levels[i], size * sizeof(unsigned long));
        if (!shrink) {
...
{
    printf("----test hbitmap-----\n"); 
    HBitmap* map = hbitmap_alloc(1024, 2);    
    
    /*0, 1, 2, 3 all set, because granularity is 2^2*/
    hbitmap_set(map, 0 , 1);
    
    /*0,1,2,3 is true*/
    bool ret = hbitmap_get(map, 0);
    printf("ret=%d \n", ret); 
    ret = hbitmap_get(map, 1);
    printf("ret=%d \n", ret); 
    ret = hbitmap_get(map, 2);
    printf("ret=%d \n", ret); 
    ret = hbitmap_get(map, 3);
    printf("ret=%d \n", ret); 

    /*4 is false*/
    ret = hbitmap_get(map, 4);
    printf("ret=%d \n", ret); 
