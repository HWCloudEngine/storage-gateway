...
class InnerRpcClient : public RpcClient {
 public:
    ...
    StatusCode get_writable_journals(const std::string& uuid, const std::string& vol,
                               const int limit,
                               list<JournalElement>* journal_list);
    StatusCode seal_journals(const std::string& uuid, const std::string& vol,
                       const std::list<std::string>& list_);
    StatusCode update_producer_marker(const std::string& uuid, const std::string& vol,
                                const JournalMarker& marker);
    StatusCode update_multi_producer_markers(const std::string& uuid,
                            const map<std::string, JournalMarker>& markers);

    StatusCode get_journal_marker(const std::string& vol_id, JournalMarker* marker);

    StatusCode get_journal_list(const std::string& vol_id,
                          const JournalMarker& marker, const int limit,
                          list<JournalElement>* journal_list);

    StatusCode update_consumer_marker(const JournalMarker& marker,
