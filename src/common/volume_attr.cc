...
{
    // todo:remove the condition check if not update frequently
    if(vol_info.vol_status() != m_vol_info.vol_status()
        || vol_info.rep_status() != m_vol_info.rep_status()
        || vol_info.role() != m_vol_info.role()){
