...
typedef struct cmp{
    bool operator()(std::shared_ptr<RepVolume> a,std::shared_ptr<RepVolume> b){
        system_clock::time_point now = system_clock::now();
        if(now - a->get_last_task_tp() > DEADLINE || now - b->get_last_task_tp() > DEADLINE)
            return a->get_last_task_tp() > b->get_last_task_tp(); // earlier has higher priority
        ...
        return a->get_last_task_tp() < b->get_last_task_tp();
    }
}cmp;
...
Replicate::Replicate(std::shared_ptr<CephS3Meta> meta,
        ...
        running_(true),
        enable_(true),
        despatch_pool_(new sg_threads::ThreadPool(DESPATCH_THREAD_CNT)){
...
int Replicate::add_volume(const string& vol_id){
    ...
    lck.unlock();
    std::shared_ptr<RepVolume> state(new RepVolume(uuid_,vol_id,mount_path_,meta_));
    LOG_INFO << "add volume " << vol_id;
    lck.lock();
    volumes_.insert(std::pair<std::string,std::shared_ptr<RepVolume>>(vol_id,state));
    return 0;
}
    ...
    */
    while(CLIENT_READY != client_->get_state(false) && running_){
        ...
        LOG_INFO << "replicate client state2:" 
            << client_->get_printful_state(s);
        if(CLIENT_FAILURE == s || CLIENT_SHUTDOWN == s){
            std::this_thread::sleep_for(std::chrono::seconds(5));
        }
    };
}

void Replicate::despatch(std::shared_ptr<RepVolume> rep_vol){
    ...
    if(!task)
        return;
    client_->submit_task(task,callback);
}
...
void Replicate::run(){
    ...
    while(running_){
        ...
        std::function<void(Replicate*,std::shared_ptr<RepVolume>)> f = &Replicate::despatch;
        std::priority_queue<std::shared_ptr<RepVolume>,std::vector<std::shared_ptr<RepVolume>>,cmp> queue;
        std::unique_lock<std::mutex> lck(volume_mtx_);
        for(auto it=volumes_.begin();it!=volumes_.end();++it){
            queue.push(it->second);
        }
        lck.unlock();
        despatch_pool_->submit(std::bind(f,this,queue.top()));
