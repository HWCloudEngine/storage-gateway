...
static size_t _cal_data_size(off_len_t* off_len, int count)
{
    size_t data_size = 0;
...
int Bcache::_journal_read(string jfile, off_t off , shared_ptr<ReplayEntry>& out)
{
    int ret = 0;

    /*todo: optimize, must multiplex*/
    int jfile_fd = open(jfile.c_str(), O_RDONLY);
    assert(jfile_fd != -1);
    ...
    do {
        ...
        off_t start = off;
        int ret = pread(jfile_fd,(char*)&log_head, head_size, start);
        ...
        off_len = (off_len_t*)malloc(off_len_size);
        ret = pread(jfile_fd, (char*)off_len, off_len_size, start);
        if(ret != off_len_size){
        ...
        entry_data = (char*)malloc(entry_data_len);
        memcpy(entry_data, &log_head, head_size);
        memcpy(entry_data+head_size, off_len, off_len_size);
        ret = pread(jfile_fd, entry_data+head_size+off_len_size, data_size, start);
        if(ret != data_size){
            cout << "read log data failed ret=" << ret << endl;
            ret = -1;
            break;
        }
        out.reset(new ReplayEntry(entry_data, entry_data_len, IoVersion(0,0)));
        ret = 0;
    }while(0);
                            ...
                            const vector<Bkey>& hit_keys)
{
    ...
    for(int i = 0; i < hit_keys.size(); i++){
        ...
        if(cache_type == CEntry::IN_MEM){
            ...
            //todo:open log file and read 
            int ret = _journal_read(log_file, log_off, log_head);
            if(ret){
                cout << "[bcache] journal read failed " << endl;
                return ret;
            }
        } else {
            assert(0);
