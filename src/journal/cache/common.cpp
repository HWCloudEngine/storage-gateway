...
size_t IReadFile:: read_entry(off_t off, shared_ptr<ReplayEntry>& entry)
{
    ...
    do {
        ...
        entry.reset(new ReplayEntry(entry_data, entry_data_len, IoVersion(0,0)));
        ret = entry_data_len;  
    }while(0);
...
{
    if(m_fd != -1){
        cout << "open " << m_file.c_str() << "already opend" << endl;
        return m_fd;
    }
