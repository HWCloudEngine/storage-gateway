...
public:
    static ControlService* s_instance;
    static mutex* s_mutex;

    static ControlService* GetInstance(map<string, shared_ptr<Volume>>& volumes){
        if(nullptr == s_instance){
            lock_guard<std::mutex> lock(*s_mutex); 
            if(nullptr == s_instance){
                s_instance = new ControlService(volumes); 
            }
        } 
        LOG_INFO << "ControlService GetInstance";
        return s_instance;
    }
