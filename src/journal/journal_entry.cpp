...
uint32_t JournalEntry::calculate_crc()
{
    ...
    this->crc = crc32c(data.c_str(), data.size(), initial);
    return this->crc;
}
...
size_t JournalEntry::persist(int fd, off_t off)
{
    ...
    size_t write_size = sizeof(type)+sizeof(length)+data.size()+sizeof(crc);
    size_t write_ret  = pwritev(fd, iov, 4, off);
    assert(write_size == write_size);
    
    LOG_INFO << "persist type:" << type << " length:" << length << " crc:" << crc;
    return write_ret;
}
...
size_t JournalEntry::persist(FILE* file, off_t off)
{
    ...
    memcpy(write_buf, (char*)data.c_str(), length);
    write_buf += length;
...
bool JournalEntry::serialize(string& out)
{
