...
private:
    ...
    /*todo optimize 
     * this structure not store message serialize buffer,
     * we have to serialize twice:
     *    1. when calculate crc 
     *    2. when persist to journal file
     * solution:
     *    refactor cache, make CEntry point to data only, not refer JournalEntry
     */
};
