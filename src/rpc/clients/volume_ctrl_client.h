...
using huawei::proto::control::UpdateVolumeStatusReq;
using huawei::proto::control::UpdateVolumeStatusRes;
...
public:
    ...
    StatusCode update_volume_status(const std::string& volume_id,
            const VOLUME_STATUS& status)
    {
        UpdateVolumeStatusReq req;
        req.set_volume_id(volume_id);
        req.set_status(status);
        ClientContext context;
        UpdateVolumeStatusRes res;

        Status status = stub_->UpdateVolumeStatus(&context, req, &res);
        if (!status.ok())
        {
            return sInternalError;
        }
