...
Jcache::Iterator::Iterator(SharedMutex& lock, map<Jkey, shared_ptr<CEntry>>::iterator it)
    : m_lock(lock), m_it(it) {
}
...
Jcache::Iterator Jcache::Iterator::operator++(int) {
    ReadLock read_lock(m_lock);
    Jcache::Iterator prev(m_lock, m_it);
    m_it++;
    return prev;
}
...
Jcache::Iterator Jcache::begin() {
    return Jcache::Iterator(m_mutex, m_cache.begin());
}

Jcache::Iterator Jcache::end() {
    return Jcache::Iterator(m_mutex, m_cache.end());
}
