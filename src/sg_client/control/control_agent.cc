...
bool AgentControlImpl::agent_add_device(std::string device)
{
	if(device.empty())
	{
		LOG_INFO << "agent add device failed,device is null";
		return false;
	}
	struct agent_ioctl_add_dev info;
	info.dev = strdup(device.c_str());
		...
		control::EnableSGRes* res)
{
	...
	if(agent_add_device(dev_name))
	{
		persist_device(dev_name);
...
bool AgentControlImpl::persist_device(std::string device)
{
