...
JournalWriter::JournalWriter(BlockingQueue<shared_ptr<JournalEntry>>& write_queue,
    ...
    :thread_ptr(), write_queue_(write_queue), reply_queue_(reply_queue),
     vol_attr_(vol_attr), cur_file_ptr(NULL), cur_journal_size(0),
...
bool JournalWriter::write_journal_header() {
    ...
    if (fwrite(&journal_header, sizeof(journal_file_header_t),
              1, cur_file_ptr) != 1) {
        LOG_ERROR << "write journal header faied,journal:"
                  << cur_lease_journal.second.path()
