...
JournalWriter::JournalWriter(BlockingQueue<shared_ptr<JournalEntry>>& write_queue,
    ...
    :thread_ptr(),
     ...
     producer_marker_is_changed(false)
{
}
 ...
 
{
    ...
    // register event
    DR_ASSERT(0 == producer_event.register_to_epoll_fd(epoll_fd));
...
int JournalWriter::update_producer_marker(){
    std::unique_lock<std::mutex> lck(producer_mtx);
    JournalMarker marker = cur_producer_marker;
    producer_marker_is_changed = false;
    lck.unlock();

    if(false == rpc_client->update_producer_marker(
            lease_client_->get_lease(),vol_id,marker)){
        // restore this flag if update failed
        LOG_ERROR << "update volume[" << vol_id << "] producer marker failed!";
        producer_marker_is_changed = true;
        return -1;
    }
    return 0;
}
...
bool JournalWriter::is_producer_marker_need_update(){
    if(producer_marker_hold_flag.load() == true)
        return false;
    return producer_marker_is_changed.load();
}
    ...
    while(true)
    {
        ...
        while(!success && (difftime(end,start) < config.write_timeout))
        {
            ...
            if(cur_file_ptr == nullptr
                ...
                DR_ASSERT(true == write_journal_header());
            }
