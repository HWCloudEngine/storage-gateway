...
bool Volume::init() {
    ...
    rep_proxy_.reset(new ReplicateProxy(conf_, vol_attr_.vol_name(),
    ...
    if (!writer_->init(conf_, idproxy_, cacheproxy_, snapshotproxy_,
    ...
    if (!replayer_->init(conf_, idproxy_, cacheproxy_, snapshotproxy_,
                         rep_proxy_)) {
