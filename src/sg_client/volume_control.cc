...
Status VolumeControlImpl::UpdateVolumeStatus(ServerContext* context,
        const control::UpdateVolumeStatusReq* req,
        control::UpdateVolumeStatusRes* res)
{
    std::string volume_id = req->volume_id();
    VOLUME_STATUS status = req->status();
    bool result = update_volume_status(volume_id, status);
    if (result)
    {
        res->set_status(StatusCode::sOk);
        return Status::OK;
