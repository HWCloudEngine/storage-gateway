                                      ...
                                      const boost::system::error_code& e)
{
    ...
    BlockDevice blk_dev(dev_path);
    size_t dev_size = blk_dev.dev_size();
    LOG_INFO << "add volume:" << vol_name << " dev_path:" << dev_path
             << " dev_size:" << dev_size;
    
    StatusCode ret = vol_inner_client_->create_volume(vol_name, dev_path, dev_size, 
                                                      VolumeStatus::VOL_ENABLING);
    if(ret != StatusCode::sOk){
        send_reply(client_sock, req_head_buffer, req_body_buffer, false);
        return;
    }

    ret = vol_inner_client_->update_volume_status(vol_name, VolumeStatus::VOL_AVAILABLE);
    if(ret != StatusCode::sOk){
        send_reply(client_sock, req_head_buffer, req_body_buffer, false);
        return;
    }
    ...
    ret = vol_inner_client_->get_volume(vol_name, volume_info);
    if(ret != StatusCode::sOk){
        send_reply(client_sock, req_head_buffer, req_body_buffer, false);
        return;
    }
