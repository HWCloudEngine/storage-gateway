    ...
    while(running_){
        ...
        if(ret > 0){
            for(int i=0;i<ret;i++){
                ...
                DR_ASSERT(writer!=nullptr);
                writer->clear_producer_event();
                if(writer->is_producer_marker_need_update()){
                    DR_ASSERT(writer->update_producer_marker() == 0);
                }
            }
...
int  VolumeManager::update_all_producer_markers(){
    std::map<string,JournalMarker> markers;
    for(auto it=volumes.begin();it!=volumes.end();it++){
        std::shared_ptr<JournalWriter> writer = it->second->get_writer();
        if(writer->is_producer_marker_need_update()){
            JournalMarker marker = writer->get_cur_producer_marker();
            markers.insert(std::pair<string,JournalMarker>(it->first,marker));
        }
    }
    if(markers.empty())
        return 0;
    bool res = writer_rpc_client->update_multi_producer_markers(
            lease_client->get_lease(),markers);
    if(res){
        LOG_DEBUG << "update all producer marker when timeout.";
        return 0;
    }
    else{
