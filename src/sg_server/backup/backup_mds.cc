                                     ...
                                     const BackupType& btype)
{
    ...
    LOG_INFO << "prepare create vname:" << m_ctx->vol_name() << " bname:" << bname;
    ...
    /*db persist */
    string pkey = spawn_backup_attr_map_key(bname);
    string pval = spawn_backup_attr_map_val(cur_backup_attr);
    m_ctx->index_store()->db_put(pkey, pval);
 
    LOG_INFO << "prepare create vname:" << m_ctx->vol_name() << " bname:" << bname << " ok";
...
StatusCode BackupMds::prepare_delete(const string& bname)
{
    ...
    /*delete backup*/
    {
        ...
        string pkey = spawn_backup_attr_map_key(bname);
        string pval = spawn_backup_attr_map_val(it->second);
        m_ctx->index_store()->db_put(pkey, pval);
    }
...
StatusCode BackupMds::local_restore(const string& bname, ServerWriter<RestoreBackupInAck>* writer)
{
    string backup_base = m_ctx->get_backup_base(bname);
...
StatusCode BackupMds::remote_restore(const string& bname, ServerWriter<RestoreBackupInAck>* writer)
{
                                             ...
                                             RemoteBackupStartAck* ack)
{
    ...
    if(it == m_ctx->cur_backups_map().end()){
        LOG_INFO << "do remote create start vname:" << m_ctx->vol_name() <<" bname:" 
...
StatusCode BackupMds::do_remote_create_upload(UploadDataReq* req, UploadDataAck* ack)
{
    ...
    int write_ret = m_ctx->block_store()->write(blk_obj, (char*)blk_data, blk_len, blk_off);
    assert(write_ret == 0);
                                       ...
                                       RemoteBackupDeleteAck* ack)
{
    ...
    string vol_name = req->vol_name();
    string backup_name = req->backup_name();
    LOG_INFO << "do remote delete vname:" << m_ctx->vol_name() << " bname:" << backup_name;
...
int BackupMds::do_task_schedule()
{
    ...
    while(m_task_schedule_run)
    {
        ...
        
        {
            ...
            {
                if((*it)->ready()){
                    LOG_INFO << "task:" << (*it)->task_name() << " ready will run";
        ...
        {
            if((*it)->finish()){
                LOG_INFO << "task:" << (*it)->task_name() << " finish will remove";
