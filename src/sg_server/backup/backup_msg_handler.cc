...
BackupMsgHandler::BackupMsgHandler(BackupMgr& backup_mgr) : m_backup_mgr(backup_mgr)
{
...
StatusCode BackupMsgHandler::handle_remote_create_start(TransferRequest* req, RpcIoStream* stream)
{
    ...
    StatusCode ret = m_backup_mgr.handle_remote_create_start(&start_req, & start_ack);
...
StatusCode BackupMsgHandler::handle_remote_create_upload(TransferRequest* req, RpcIoStream* stream)
{
    ...
    StatusCode ret = m_backup_mgr.handle_remote_create_upload(&upload_req, &upload_ack);
    return ret;
}
