...
LocalCreateTask::LocalCreateTask(const string& backup_name, shared_ptr<BackupCtx> ctx) 
...
StatusCode LocalCreateTask::do_incr_backup()
{
    ...
    string cur_snap = backup_to_snap_name(cur_backup);
    if(!m_ctx->is_snapshot_valid(pre_snap) || !m_ctx->is_snapshot_valid(cur_snap)){
...
RemoteCreateTask::RemoteCreateTask(const string& backup_name, shared_ptr<BackupCtx> ctx) 
...
StatusCode RemoteCreateTask::do_incr_backup()
{
    ...
    vector<DiffBlocks> diff_blocks;
    ret_code = m_ctx->snap_client()->DiffSnapshot(m_ctx->vol_name(), pre_snap, cur_snap, 
                                                  diff_blocks); 
    assert(ret_code == StatusCode::sOk);

    /*read diff data(current snapshot and diff region)*/
    size_t chunk_size = COW_BLOCK_SIZE;
    char* chunk_buf = (char*)malloc(COW_BLOCK_SIZE);
    for(auto it : diff_blocks)
    {
        uint64_t diff_block_no_size = it.diff_block_no_size() ;
        for(int i = 0; i < diff_block_no_size; i++)
...
LocalDeleteTask::LocalDeleteTask(const string& backup_name, shared_ptr<BackupCtx> ctx)
                 :AsyncTask(backup_name, ctx)
{
    m_task_name.append(backup_name);
