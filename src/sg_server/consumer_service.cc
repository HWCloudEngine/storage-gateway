...
Status ConsumerServiceImpl::GetJournalMarker(ServerContext* context,
    ...
    RESULT res = _meta->get_journal_marker(request->uuid(),request->vol_id(),request->type(),marker);
    if(res != DRS_OK) {
...
Status ConsumerServiceImpl::GetJournalList(ServerContext* context,
    ...
    RESULT res = _meta->get_consumable_journals(request->uuid(),request->vol_id(),marker,
            request->limit(), list);
    if(res != DRS_OK) {
