...
std::shared_ptr<RepTask> RepVolume::get_next_task(){
    ...
    if(base_sync_state_ == NEED_SYNC){
        ...
        // whether base snapshot is created
            ...
            StatusCode ret = SnapReader::instance().get_client()->QuerySnapshot(
...
std::shared_ptr<RepTask> RepVolume::generate_base_sync_task(
                    const string& pre_snap,const string& cur_snap){
    std::shared_ptr<RepTask> task(new DiffSnapTask(pre_snap,cur_snap));
    task->set_id(0);
    task->set_vol_id(vol_id_);
    task->set_status(T_WAITING);
    // TODO:set journal key
    auto record_it = vol_meta_.records().rbegin();
    int64_t counter;
    sg_util::extract_counter_from_object_key(record_it->marker().cur_journal(),counter);
    task->set_j_counter(counter);
//    task->set_end_off();
    task->set_callback(std::bind(
        &RepVolume::recycle_base_sync_task,this,std::placeholders::_1));
    task->set_is_opened(false);
    task->set_ts(time(nullptr));
    task->init();
    return task;
}
void RepVolume::recycle_base_sync_task(std::shared_ptr<RepTask> t){
    // TODO:
