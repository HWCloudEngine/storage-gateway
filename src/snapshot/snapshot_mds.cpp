...
int SnapshotMds::get_uid(const GetUidReq* req, GetUidAck* ack)
{
    string vol_name = req->vol_name();
    LOG_INFO << "SnapshotMds get uid"
             << " vol_name:" << vol_name;
...
int SnapshotMds::get_snapshot_name(const GetSnapshotNameReq* req,
                                   GetSnapshotNameAck* ack)
{
    string vol_name  = req->vol_name();
    snapid_t snap_id = req->snap_id();
    LOG_INFO << "SnapshotMds get snapshot name"
             << " vol_name:" << vol_name
             << " snap_id:"  << snap_id;
    string snap_name; 
    ...
    assert(!snap_name.empty());   
    ack->set_ret(SNAPSHOT_OK);
    ack->set_snap_name(snap_name);
    LOG_INFO << "SnapshotMds get snapshot name"
             << " vol_name:" << vol_name
             << " snap_id:"  << snap_id
             << " snap_name:" << snap_name
...
int SnapshotMds::get_snapshot_id(const GetSnapshotIdReq* req, 
                                 GetSnapshotIdAck* ack)
{
    string vol_name  = req->vol_name();
    string snap_name = req->snap_name();
    LOG_INFO << "SnapshotMds get snapshot id"
             << " vol_name:" << vol_name
             << " snap_name:"  << snap_name;
    
    snapid_t snap_id = m_snapshots[snap_name];
...
int SnapshotMds::create_snapshot(const CreateReq* req, CreateAck* ack)
{
    ...
    string   snap_name = req->snap_name();
    snapid_t snap_id   = req->snap_id();
    LOG_INFO << "SnapshotMds create snapshot" 
             <<" vname:" << vol_name 
             <<" sname:" << snap_name
             <<" snid:"  << snap_id;
    ...
    if(it != m_snapshots.end()){
     ...
     /*in db update snapshot */
    string pkey = spawn_snapshot_map_key(snap_name);
    string pval = to_string(snap_id);
    m_index_store->db_put(pkey, pval);
    /*in memroy update snapshot*/
    m_snapshots.insert(pair<string, snapid_t>(snap_name, snap_id));

     /*in db update snapshot status*/
    pkey = spawn_status_map_key(snap_id);
    m_index_store->db_put(pkey, to_string(huawei::proto::SNAPSHOT_CREATED));
    /*in db update snapshot status*/
    m_snapshots_status.insert(pair<snapid_t, snapshot_status_t>(snap_id, 
                              huawei::proto::SNAPSHOT_CREATED));
...
{
    string vol_name  = req->vol_name();
    LOG_INFO << "SnapshotMds list snapshot"
             << " vname:" << vol_name;
...
int SnapshotMds::delete_snapshot(const DeleteReq* req, DeleteAck* ack)
{
    ...
    /*snapshot status*/
    string pkey = spawn_status_map_key(snap_id);
    m_index_store->db_del(pkey);
    m_snapshots_status.erase(snap_id);

    /*snapshot table*/
    for(auto it : m_snapshots){
        if(it.second == snap_id){
            string pkey = spawn_snapshot_map_key(it.first);
            m_index_store->db_del(pkey);
            m_snapshots.erase(it.first);
            break;
        }
    }
...
int SnapshotMds::rollback_snapshot(const RollbackReq* req, RollbackAck* ack)
{
    ...
    LOG_INFO << "SnapshotMds rollback snapshot"
             << " vname:"   << vol_name
             << " snap_id:" << snap_id;
...
int SnapshotMds::create_update_status(snapid_t snap_id, 
                                      snapshot_status_t snap_status)
{
   return SNAPSHOT_OK;
}

int SnapshotMds::delete_update_status(snapid_t snap_id, 
                                      snapshot_status_t snap_status)
{
    lock_guard<std::mutex> lock(m_mutex);
    auto it = m_snapshots_status.find(snap_id);
    if(it == m_snapshots_status.end()){
        return SNAPSHOT_NOEXIST; 
    }
    it->second = snap_status;
    
    /*in db update snapshot status*/
    string pkey = spawn_status_map_key(snap_id);
    m_index_store->db_put(pkey, to_string(snap_status));
    return SNAPSHOT_OK;
}

int SnapshotMds::set_status(const SetStatusReq* req, SetStatusAck* ack)
{
    string   vol_name  = req->vol_name();
    uint64_t snap_id   = req->snap_id();
    snapshot_status_t snap_status = req->snap_status();

    LOG_INFO << "SnapshotMds set status"
             << " vname:"       << vol_name 
             << " snap_id:"     << snap_id
             << " snap_status:" << snap_status;

    if(snap_status == ::huawei::proto::SNAPSHOT_CREATED){
        create_update_status(snap_id, snap_status);
    } else if(snap_status == ::huawei::proto::SNAPSHOT_DELETING){
        delete_update_status(snap_id, snap_status);
    } else {
        ;
    } 
...
int SnapshotMds::get_status(const GetStatusReq* req, GetStatusAck* ack)
{
    string   vol_name  = req->vol_name();
    uint64_t snap_id   = req->snap_id();
    LOG_INFO << "SnapshotMds get status"
             << " vname:" << vol_name 
             << " snap_id:" << snap_id;
...
int SnapshotMds::cow_op(const CowReq* req, CowAck* ack)
{
    ...
    if(blk_it != block_map.end()){
        ...
        ack->set_ret(SNAPSHOT_OK);
        ack->set_op(::huawei::proto::COW_NO);
        return SNAPSHOT_OK;
    }
    ...
    ack->set_ret(SNAPSHOT_OK);
    ack->set_op(::huawei::proto::COW_YES);
    ack->set_cow_blk_object(cow_object);

    LOG_INFO << "SnapshotMds cow_op COW_YES "
             << " cow_object:" << cow_object;
    return SNAPSHOT_OK;
}
...
void SnapshotMds::trace()
{
    ...
    for(auto it : m_snapshots_status){
        LOG_INFO << "\t\t snap_id:" << it.first << " snap_status:" << it.second; 
    }
...
int SnapshotMds::cow_update(const CowUpdateReq* req, CowUpdateAck* ack)
{
    ...
    LOG_INFO << "SnapshotMds cow_update"
             << " vname:"   << vname 
             << " snap_id:" << snap_id
             << " blk_id:"  << blk_no
             << " cow_obj:" << cow_obj;
    ...
    string pkey = spawn_cow_block_map_key(snap_id, blk_no);
    ...
    string pval = spawn_cow_object_map_val(cow_obj_ref);
    m_index_store->db_put(pkey, pval);
...
string SnapshotMds::spawn_key(const string& prefix, const string& value)
{
    string key = prefix;
    key.append(FS);
    key.append(value);
...
void SnapshotMds::split_key(const string& raw_key, string& prefix, string& key)
{
...
string SnapshotMds::spawn_snapshot_map_key(const string& snap_name)
{
    return spawn_key(SNAPSHOT_MAP_PREFIX, snap_name);
}

void SnapshotMds::split_snapshot_map_key(const string& raw_key, 
                                         string& snap_name)
{
    string prefix;
...
string SnapshotMds::spawn_status_map_key(const snapid_t& snap_id)
{
    return spawn_key(SNAPSHOT_STATUS_PREFIX, to_string(snap_id));
}

void SnapshotMds::split_status_map_key(const string& raw_key, 
                                         snapid_t& snap_id)
{
    string prefix;
    string key;
    split_key(raw_key, prefix, key);
    snap_id = atol(key.c_str());
}

string SnapshotMds::spawn_cow_block_map_key(const snapid_t& snap_id,
...
void SnapshotMds::split_cow_block_map_key(const string& raw_key, 
                                          snapid_t& snap_id, 
                                          block_t& block_id)
{
    string prefix;
...
string SnapshotMds::spawn_cow_object_map_key(const string& obj_name)
{
    return spawn_key(SNAPSHOT_COWOBJECT_PREFIX, obj_name);
}

void SnapshotMds::split_cow_object_map_key(const string& raw_key, 
                                           string obj_name)
{
   string prefix;
...
string SnapshotMds::spawn_cow_object_map_val(const cow_object_ref_t& obj_ref)
{
    string val;
    for(auto it : obj_ref){
...
void SnapshotMds::split_cow_object_map_val(const string& raw_val, 
                                           cow_object_ref_t& obj_ref)
{
    string raw = raw_val;
...
int SnapshotMds::diff_snapshot(const DiffReq* req, DiffAck* ack)
{
    ...
    LOG_INFO << "SnapshotMds diff snapshot"
             << " vname:"   << vname 
    ...
    snapid_t first_snapid = m_snapshots[first_snap];
    snapid_t last_snapid  = m_snapshots[last_snap];
...
int SnapshotMds::read_snapshot(const ReadReq* req, ReadAck* ack)
{
    ...
    LOG_INFO << "SnapshotMds read snapshot"
             << " vname:" << vol_name
             << " sname:" << snap_name
             << " off:"  << off
             << " len:"  << len;
