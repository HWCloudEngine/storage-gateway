...
case $1 in
    'start')
        ...
        ssh -T ${LOCAL_HOST} "cd ${SG_SCRIPTS_HOME}; ./sg_services.sh start"
        res1=$?
        ssh -T ${REMOTE_HOST} "cd ${SG_SCRIPTS_HOME}; ./sg_services.sh start"
        res2=$?
        ;;
    'stop')
        sg_print "stop sg process"
        ssh -T ${LOCAL_HOST} "cd ${SG_SCRIPTS_HOME}; ./sg_services.sh stop"
        res1=$?
        ssh -T ${REMOTE_HOST} "cd ${SG_SCRIPTS_HOME}; ./sg_services.sh stop"
        res2=$?
        ;;
    'clean')
