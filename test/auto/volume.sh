...
usage="usage: $0 [action:enable|disable] [volume id]"
TEST_VOLUME_PATH="/dev/sdb"
#1G
...
case $1 in
    'enable')
        ...
        res=$(python control.py volume -v ${TEST_VOLUME_ID} -p ${TEST_VOLUME_PATH} -s ${TEST_VOLUME_SIZE} -a enable)
        sg_print "$res"
