...
int get_f(CephS3Api& s3_meta, const char* key){
    string value;
    RESULT res = s3_meta.get_object(key,&value);
    if(RESULT::DRS_OK != res){
...
int delete_key(CephS3Api& s3_meta, const char* key){
    if(RESULT::DRS_OK == s3_meta.delete_object(key)){
...
int delete_all_keys(CephS3Api& s3_meta){
    ...
    RESULT res = s3_meta.list_objects(NULL,NULL,0,&list,NULL);
    if(RESULT::DRS_OK == res){
